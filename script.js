...
class ChatApp {
    ...
    // === Settings & Auth stubs (to avoid runtime errors on GH Pages) ===
    loadSettings() {
        try {
            const lsKey = localStorage.getItem('chatApiKey');
            const lsId = localStorage.getItem('chatApiId');
            this.apiKey = (lsKey && lsKey !== 'api-key') ? lsKey : '';
            this.apiId = (lsId && lsId !== 'api-key') ? lsId : '';
            if (this.apiKeyInput) this.apiKeyInput.value = this.apiKey || '';
            if (this.apiIdInput) this.apiIdInput.value = this.apiId || '';
        } catch (_) {}
    }

    saveSettings() {
        const key = (this.apiKeyInput?.value || '').trim();
        const id = (this.apiIdInput?.value || '').trim();
        try {
            localStorage.setItem('chatApiKey', key || '');
            localStorage.setItem('chatApiId', id || '');
        } catch (_) {}
        this.apiKey = key;
        this.apiId = id;
    ...
    updateSubscriptionStatus() {
        if (this.subscriptionStatus) {
            this.subscriptionStatus.querySelector('.plan-name').textContent = 'Dragon';
        }
    }
    ...
    handlePhotoUpload(e) {
        ...
        if (!file) return;
        this.showNotification('Анализ изображений оффлайн пока недоступен', 'warning');
    }

    loginWithGoogle() {
        this.showNotification('Авторизация отключена на GitHub Pages', 'warning');
    }

    signupWithEmail() {
        this.showNotification('Регистрация отключена на GitHub Pages', 'warning');
    }
    ...
    async sendMessage() {
        ...
        // Подсказка: можно настроить API ключи, но это не обязательно для оффлайн-режима
        if (!this.apiKey || !this.apiId || this.apiKey === 'api-key' || this.apiId === 'api-key') {
            this.showNotification('Работаю в оффлайн-режиме (без ключей). Можно добавить ключи в настройках.', 'info');
        }
        ...
        try {
            // RAG контекст (может быть async при векторном поиске)
            const ragContext = await (this.semanticSearchContext ? this.semanticSearchContext(message, 4) : this.retrieveKnowledgeContext(message, 4));
...
<pre><code class="language-javascript">// Пример: погода по координатам (Москва)
...
- При необходимости добавлю шаги установки и запуска`;
    ...
    retrieveKnowledgeContext(query, topK = 4) {
        ...
        if (this.vectorIndex && window.transformers) {
            ...
            // semanticSearchContext — async; здесь вернем пусто, а в sendMessage оно будет вызвано отдельно
            return '';
        }
